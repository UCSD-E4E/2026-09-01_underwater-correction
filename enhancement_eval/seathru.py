...
doing after gray-world, white-patch and per-channel stretch all failed: beta
comes from the slate fit (`attenuation.py`, validated against the published
pure-water absorption spectrum across seven dives), and z from
`LaserDepth.range_m`. A measured beta cannot over-correct into magenta the way
a statistical white balance does, because it is not inferred from the picture
it is correcting.

**The limitation is range, and it is real.** Sea-thru proper needs a per-pixel
depth map; this corpus has range at exactly one pixel, so `remove_water` treats
z as uniform across the frame. That is right for the fish the laser is on and
increasingly wrong for background at another distance. Scaling a monocular
depth estimate by the laser's metric range is the step that would lift this
restriction; it is not implemented here.

**What to expect.** The correction scales with range, so at the 1-1.5 m of the
current field frames it is modest -- the fitted beta implies gains of about
...
tone treatment, not from the physics; the physics earns its place further out.
"""
...
__all__ = ["estimate_veil", "remove_water"]
...
def estimate_veil(
    ...
    `linear_rgb` must be linear and in [0, 1]: the model is written in radiance,
    and estimating a veil through a gamma curve would return a number that is
    not the veil.
    """
    return tuple(
...
def remove_water(
    ...
    *,
    ...
    Args:
        linear_rgb: linear RGB in [0, 1]. Applying this after gamma or CLAHE
            would invert a curve that is not in the model.
        beta: per-channel attenuation, per metre, in (R, G, B) order.
        range_m: the laser's metric range, applied uniformly -- see the module
            docstring on what that costs.
        veil: backscatter per channel; estimated from the dark tail when None.
        normalize: rescale the result to [0, 1]. Off for tests and for anyone
            who wants the radiance ratios preserved exactly.

    Returns:
        The direct-signal estimate, same shape and dtype-family as the input.
    """
    if range_m < 0:
    ...
    transmission = np.exp(-np.asarray(beta, dtype=np.float64) * float(range_m))
